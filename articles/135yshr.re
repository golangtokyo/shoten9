...
LSB法は、Least Significant Bitを略したもので画像データのビットを隠したいデータで置き換える方法です。置き換えるビット数が多いほど、一枚の画像に隠すことはできるデータ量は増えます。しかし、画像の見た目が変わってしまうので、見た人は違和感を感じ何か隠していることがバレてしまいます。
それらのことを考慮し、今回作成するプログラムでは、使用する画像形式や隠すデータの形式から下位2ビットを置き換えて重要なデータを隠すことにしました。

=== PNGフォーマット
プログラムを作成するにあたり、実装前にPNGフォーマットについても簡単に説明します。

PNGは、Portable Network Graphicsの略で、"ピング"や"ピン"と呼ばれており、GIFに変わる画像フォーマットとして開発されました。
