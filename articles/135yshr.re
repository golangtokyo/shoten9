...
LSB法は、Least Significant Bit を略したもので画像データのビットを隠したいデータで置き換える方法です。置き換えるビット数が多いほど、一枚の画像に隠すことはできるデータ量は増えます。しかし、画像の見た目が変わってしまうので、見た人は違和感を感じ何か隠していることがバレてしまいます。使用する画像形式や隠すデータの形式を考慮して下位2ビットを置き換えて重要なデータを隠します。
...
IHDRチャンクは、ヘッダ情報を配置する領域で画像の幅や画像の高さ圧縮形式などPNG画像に関する情報が保存されます。
...
@<list>{hide_data_in_image}に示すencode関数を使って画像ファイルにデータを隠します。
前提条件として、画像データはデコード済みであり、隠したいデータは @{[]byte} に変換済みである必要があります。
