...
=== インストール
インストールするには、Go の実行環境がインストールされている必要があります。事前に公式サイト(https://golang.org/) を参考にインストールしておいてください。
...
もっと大きなデータを隠せるようにしたいとか、PNG 以外のファイルにも対応したいというかは、GitHubのリポジトリをForkしてどんどん機能拡張して楽しんでいただけると、うれしいです。
